...
/*   Updated: 2023/02/14 07:50:22 by luntiet-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_child(t_options *o, t_parse_cmd *cmd, int *fd, int *pipefd)
{
	...
	if (fd[0] == -1)
		safe_quit_open_error(o, fd, pipefd, cmd->infile);
	close(pipefd[0]);
	if (o->pipes > 0)
...
int	run_pipe(t_options *o, int *i, int *fd, pid_t *last_child)
{
	...
	if (o->tables[*i]->out == WRITE || o->tables[*i]->out == APPEND)
	{
		if (fd[1] != STDOUT_FILENO)
			close(fd[1]);
