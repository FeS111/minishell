  ...
  while (o->tables[++i]) {
      ...
      while (o->tables[i]->cmd->args[k] &&
        ...
        if (o->tables[i]->cmd->args[k][j] == '$')
          o->tables[i]->cmd->args[k] =
              replace_variable(o, o->tables[i]->cmd->args[k], j,
                               get_varlength(&o->tables[i]->cmd->args[k][j]));
        else if (o->tables[i]->cmd->args[k][j] == '~')
          o->tables[i]->cmd->args[k] =
              replace_home(o->tables[i]->cmd->args[k], j);
      }
    ...
    while (o->tables[i]->cmd->cmd[++k] != '\0') {
      ...
      if (o->tables[i]->cmd->cmd[k] == '$')
        o->tables[i]->cmd->cmd =
            replace_variable(o, o->tables[i]->cmd->cmd, k,
                             get_varlength(&o->tables[i]->cmd->cmd[k]));
